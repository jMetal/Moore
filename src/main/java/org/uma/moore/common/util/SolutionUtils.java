...
public class SolutionUtils {
  ...
  public static <S extends Solution<?>> boolean isFeasible(S solution) {
    ...
    if (((solution.getAttribute("NUMBER_OF_VIOLATED_CONSTRAINS") == null) ||
        ((int) solution.getAttribute("NUMBER_OF_VIOLATED_CONSTRAINS") == 0))) {
      result = true ;
    }
  ...
  public static <S extends Solution<?>> S getBestSolution(S solution1, S solution2, Comparator<S> comparator) {
    return getBestSolution(solution1, solution2, comparator, () -> MooreRandom.getInstance().nextDouble());
  }
