package org.uma.moore.common.util.fileoutput;
...
public class SolutionListOutput {
  ...
  public void printVariablesToFile(FileOutputContext context, List<? extends Solution<?>> solutionList) {
    ...
    try {
      if (solutionList.size() > 0) {
        ...
        for (int i = 0; i < solutionList.size(); i++) {
          for (int j = 0; j < numberOfVariables; j++) {
            bufferedWriter.write(solutionList.get(i).getVariableString(j) + context.getSeparator());
          }
      ...
      throw new MooreException("Error writing data ", e) ;
    }
  ...
  public void printObjectivesToFile(FileOutputContext context, List<? extends Solution<?>> solutionList) {
    ...
    try {
      ...
      throw new MooreException("Error printing objecives to file: ", e);
    }
  }
    ...
    try {
      if (solutionList.size() > 0) {
        ...
        if (numberOfObjectives != minimizeObjective.size()) {
          throw new MooreException("The size of list minimizeObjective is not correct: " + minimizeObjective.size()) ;
        }
        for (int i = 0; i < solutionList.size(); i++) {
          for (int j = 0; j < numberOfObjectives; j++) {
