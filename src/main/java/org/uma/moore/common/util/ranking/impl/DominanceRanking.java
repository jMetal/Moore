package org.uma.moore.common.util.ranking.impl;
...
import org.uma.moore.common.util.comparator.OverallConstraintViolationComparator;
...
public class DominanceRanking<S extends Solution<?>> implements Ranking<S> {
  ...
  public Ranking<S> computeRanking(List<S> solutionSet) {
    ...
    for (int i = 0; i < population.size(); i++) {
      if (dominateMe[i] == 0) {
        front.get(0).add(i);
        solutionSet.get(i).setAttribute("DOMINANCE_RANKING", 0);
      }
    }
      ...
      while (it1.hasNext()) {
        ...
        while (it2.hasNext()) {
          ...
          if (dominateMe[index] == 0) {
            ...
            solutionSet.get(index).setAttribute("DOMINANCE_RANKING", i);
          }
        }
      }
    }
  ...
  public List<S> getSubfront(int rank) {
    if (rank >= rankedSubPopulations.size()) {
      throw new MooreException("Invalid rank: " + rank + ". Max rank = " + (rankedSubPopulations.size() -1)) ;
    }
